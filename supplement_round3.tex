...
DeepSeek V4 & 0.812 & $<.001$ & .003 & $[0.54, 0.78]$ \\
...
\small
\textit{Note.} Uniform $p$ = fraction of null replicates $\geq \rho_{\text{EFR}}$ under the uniform-noise null. Permutation $p$ = same under the permutation null. The permutation null is the stricter test: it preserves the marginal distribution of the actual LLM output and destroys only the state-level association. Qwen2.5-72B did not show detectable incremental DCE-aligned rank information beyond the distribution-preserving permutation benchmark ($p = .179$). DeepSeek V4 and MiroThinker showed incremental DCE-aligned rank information beyond mechanical anchoring ($p = .003$ and $p < .001$, respectively). After Bonferroni correction for three models ($\alpha = .05/3 = .017$), both DeepSeek V4 and MiroThinker remain significant. All observed $\rho$ values match Table~1 of the main text to display precision.
\end{minipage}
\end{table}

The reported frontier-alignment $\rho$ values in Table~1 of the main text and Table~\ref{tab:S6} of this supplement should be read as structural diagnostics of how closely the EFR blend tracks the empirical frontier; the uniform null confirms that all three models exceed pure noise, while the permutation null distinguishes models whose LLM component shows detectable DCE-aligned rank information beyond mechanical anchoring (DeepSeek V4, MiroThinker; $p \leq .003$) from those that do not (Qwen2.5-72B; $p = .179$). The OOD frontier-alignment values ($\rho = 0.87$--$0.93$ for Qwen; $\rho = 0.87$--$0.88$ for DeepSeek V4) sit inside one SD of the tied Panel B null (mean $\rho = 0.89$, SD $= 0.06$) and should be read as descriptive structural robustness measures. The fairer predictive benchmark---held-out DCE log loss---is reported in Section~4.2 of the main text.
...
DeepSeek V4                & Provider inference API                & 2026-05-19            & 0.7    & 1.0 & 512    & 10 & Full-scale robustness; Main Table 1; Table S6 \\
...
DeepSeek V4 & EFR Anchor & MSE & 0.0024 & [0.0017, 0.0031] \\
\midrule
DeepSeek V4 & Unconstrained LLM & MVR-Wait & 0.3438 & point \\
DeepSeek V4 & EFR Anchor & MVR-Wait & 0.0729 & point \\
\midrule
DeepSeek V4 & Unconstrained LLM & Spearman \(\rho\) & $0.3025$ & [$0.0714$, $0.5123$] \\
DeepSeek V4 & EFR Anchor & Spearman \(\rho\) & $0.8123$ & [$0.6832$, $0.8885$] \\
\midrule
...
\paragraph{Qwen2.5-72B-Instruct local implementation.} This validation uses a local replication with Qwen2.5-72B via Ollama (model: \texttt{qwen2.5:72b}, quantization: Q4\_K\_M, Ollama version 0.3.0). Inference parameters: temperature 0.7, top-p 1.0, max tokens 512, 10 repetitions per task. The inference condition differs from the original Hugging Face endpoint (Table S2) in quantization and sampling implementation; exact numerical correspondence is not guaranteed. Endpoint and model identifiers for DeepSeek V4 and MiroThinker-1.7-mini are reported in Table S2.
