...
\textbf{Methods:} A Wuhan vaccination DCE ($N\!=\!1{,}027$) was combined with LLM simulation on a 64-state policy grid. EFR computes $\pi_{\text{EFR}} = \lambda\,\bar\pi_{\text{LLM}} + (1-\lambda)\,P_{\text{emp}}$ with $\lambda\!=\!0.25$ fixed as a conservative anchoring design choice, not a test-set-optimized predictive optimum; held-out log loss and Brier are reported as sensitivity diagnostics rather than as the criterion used to select $\lambda$.
...
\textbf{Limitations:} EFR requires a pre-existing DCE and was validated on a single Wuhan sample; generalization remains to be established through replication.

\textbf{Conclusion:} EFR extends an existing DCE with constrained narrative-policy evaluation by assigning most of the weight to the empirical reference; the fixed $\lambda\!=\!0.25$ anchor bounds, rather than eliminates, the deviation of LLM outputs from the DCE-derived frontier.
\end{abstract}
...
\subsection{Stage 2: Post-Hoc Adjustment Procedure} The adjustment algorithm aligns LLM outputs with the empirical reference by computing $\pi_{\mathrm{adj}}(y=1\mid\mathbf{x}_g) = \lambda \cdot \pi_{\mathrm{LLM}}(y=1\mid\mathbf{x}_g) + (1-\lambda) \cdot P_{\mathrm{emp}}(y=1\mid\mathbf{x}_g)$ (the EFR equation $\pi_{\mathrm{adj}} = \lambda \pi_{\mathrm{LLM}} + (1-\lambda) P_{\mathrm{emp}}$; full derivation in Appendix~A.1 of the Supplementary Material).
\label{sec:stage3}
...
\begin{table*}[t]
\centering
\caption{Behavioral consistency with the empirical reference before and after EFR on the 64-state grid. The empirical frontier is the corrected 6-parameter grouped/conditional logit on the full DCE sample. Spearman's $\rho$ is a diagnostic only; the held-out DCE log loss (Table~\ref{tab:lambda_heldout} of the Supplementary Material) is the primary benchmark.}
...
\item \textbf{Table S2.} \emph{Administered} DCE attribute levels: waiting time $\{0, 1, 3, 6\}$ months (4 real design levels; the raw coded data contain a single 1-row outlier at wait $= 2$ months from one respondent which we exclude as a data anomaly because it does not represent an administered design level), vaccine efficacy $\{0, 50, 70, 95\}\%$, side-effect burden $\{0, 10, 20, 30\}\%$, cash incentive $\{50, 200, 800\}$ RMB, vaccine origin $\{\text{Imported, Domestic}\}$. \emph{Note: 0 months (walk-in appointment), 0\% vaccine efficacy, 0\% side effects, and 0 RMB cash incentive appear in the empirical data only as the opt-out alternative (the 6 administered choice tasks each present two non-opt-out profiles plus an opt-out alternative that is not part of the non-opt-out DCE design matrix).} The 12 administered non-opt-out attribute combinations per respondent (1 unique design block shared by all 1027 respondents) enumerate 12 of the $3 \times 3 \times 3 \times 2 \times 4 = 216$ non-opt-out candidate cells (5.6\% coverage).
