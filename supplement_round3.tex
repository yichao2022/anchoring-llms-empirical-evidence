...
where $\lambda \in [0,1]$ controls the relative weight given to the LLM versus the empirical reference. We set $\lambda = 0.25$ as a fixed conservative design specification preserving a non-zero LLM contribution while keeping the empirical DCE dominant. The held-out DCE log loss and Brier score are reported as sensitivity diagnostics and were not used to select $\lambda$; the value is not a test-set-optimized predictive optimum.
...
\section{ELEVATE-GenAI Reporting Checklist}
...
You are a policy adviser. The respondent is deciding whether to accept the proposed COVID-19 vaccine. Read the policy description below and decide.
...
Side Effects (\% risk) & 0, 10, 20, 30 (non-opt-out: 10, 20, 30) & Negative \\
...
\item \textbf{Side-effect burden} is reported in the DCE questionnaire as $\{0, 10, 20, 30\}\%$ of respondents reporting the listed side effect, and in the 64-state grid as the integer-coded burden level $\{0, 1, 2, 3\}$; the two supports share the literal overlap $\{1, 2, 3\}$ at the coded-burden level. One code step in the grid therefore corresponds to 10 percentage points in the questionnaire (e.g., coded level 1 $=$ 10\%, level 3 $=$ 30\%). The estimated coefficient $\hat\beta_{\text{SE}}$ is reported in Table~\ref{tab:static_frontier_estimates}; the coefficient unit is the same coded burden level that enters the linear predictor.
...
\midrule
Side-effect burden & 0\%, 10\%, 20\%, 30\% & 0, 1, 2, 3 (integer-coded burden level) & per 1 coded burden level $=$ 10 percentage points & 0, 1, 2, 3 (integer-coded burden level) & 0, 1, 2, 3 \\
\midrule
...
This appendix documents the pilot experiment reported in the Pilot rows of Table~1 of the main text. The pilot served as a low-resource mechanism check rather than a directly comparable full-scale evaluation. It tested whether explicit monotonicity information could reduce frontier-relative behavioral inconsistencies in a minimal setting before scaling to the full Static-BDT Anchor implementation.
...
Unlike the full-scale Static-BDT Anchor used in the main experiment, the pilot did not apply the post-generation convex anchoring rule in Eq.~\eqref{eq:adjustment}. Instead, it used prompt-level rule injection. The pilot rows in Table~1 of the main text should therefore be interpreted as low-resource mechanism evidence that explicit monotonicity information can reduce frontier-relative behavioral inconsistencies, not as a direct estimate of the full frontier-anchored calibration mechanism.
...
To benchmark against prompt-level interventions, we evaluated a rule-injected Qwen2.5-72B-Instruct agent on the 64-state grid. To ensure a strictly fair comparison (apples-to-apples) with the unconstrained baseline reported in Table~1 of the main text, the rule-prompted condition replicated all decoding parameters identically, including the same non-zero temperature and the same number of repeated generations ($R=10$) to average out stochastic sampling noise. The only modification was the injection of explicit monotonicity rules into the system prompt.

We omit a rule-prompted comparison in this paper; rule-prompted LLM outputs are not part of the present evaluation and will be reported in a separate methodological note.
...
To assess whether the hybrid framework's performance is uniform across respondent subgroups, we replicated the held-out predictive validation on the 813 \emph{alternative-level} rows subset (from 205 held-out respondents) stratified by age, gender, and education. Table~\ref{tab:heterogeneity} reports the log loss, Brier score, and Spearman rank correlation of the Static-BDT anchored predictions within each subgroup, alongside the unconstrained Qwen2.5-72B-Instruct baseline for direct comparison. Because the 813 \emph{alternative-level} rows subset is too small for stable within-subgroup bootstrap inference, the table reports point estimates only; bootstrap intervals on the full sample are reported in the main text.

\begin{table}[h]
\centering
\caption{Subgroup heterogeneity analysis of Static-BDT ($\lambda=0.25$) performance on the 813 \emph{alternative-level} rows subset (from 205 held-out respondents), stratified by demographic characteristics.}
...
\midrule
\multirow{2}{*}{Age $<$ 55} & Unconstrained LLM & 0.835 & 0.317 & $-0.628$ \\
                        & Static-BDT       & 0.698 & 0.252 & 0.916 \\
\midrule
\multirow{2}{*}{Age $\geq$ 55} & Unconstrained LLM & 0.801 & 0.302 & $-0.617$ \\
                         & Static-BDT       & 0.701 & 0.253 & 0.922 \\
\midrule
\multirow{2}{*}{Female}    & Unconstrained LLM & 0.815 & 0.308 & $-0.627$ \\
                         & Static-BDT       & 0.690 & 0.248 & 0.897 \\
\midrule
\multirow{2}{*}{Male}      & Unconstrained LLM & 0.844 & 0.322 & $-0.622$ \\
                         & Static-BDT       & 0.710 & 0.258 & 0.937 \\
\midrule
\multirow{2}{*}{College or University} & Unconstrained LLM & 0.810 & 0.306 & $-0.560$ \\
                                & Static-BDT       & 0.694 & 0.250 & 0.950 \\
\midrule
\multirow{2}{*}{Below College} & Unconstrained LLM & 0.832 & 0.316 & $-0.642$ \\
                                & Static-BDT       & 0.700 & 0.253 & 0.906 \\
\midrule
\multirow{2}{*}{All (matched subset)} & Unconstrained LLM & 0.827 & 0.314 & $-0.626$ \\
                                & Static-BDT       & 0.699 & 0.252 & 0.917 \\
\bottomrule
\end{tabular}
\\[0.3em]
\begin{minipage}{0.95\textwidth}
\small
\textit{Note.} ``Below College'' includes middle school or below; ``College or University'' includes high school, college, or graduate degree. ``Age $<$ 55'' covers 18--54; ``Age $\geq$ 55'' covers 55--65 and 65+. The unconstrained LLM column reports Qwen2.5-72B-Instruct predictions before anchoring. The Spearman $\rho$ column reports rank correlation against the same-method Pure-DCE predictions on the same matched subset (which is the relevant anchor-reference for the LLM's direction within each stratum), not against the population frontier. Across all seven subgroups, the Static-BDT predictions achieve Spearman $\rho$ between 0.897 and 0.950 (range 0.053), indicating that the empirical anchoring benefit is approximately invariant to demographic subgroup within this Wuhan sample.
\end{minipage}
\end{table}

Two patterns emerge. First, the unconstrained LLM produces near-uniform log loss and Brier score across all subgroups (range $\leq 0.05$ on both metrics), confirming that the behavioral unreliability documented in the main text is not driven by a particular demographic segment. Second, the Static-BDT Spearman correlation against the within-stratum Pure-DCE predictions falls in a band of 0.897--0.950 across all seven subgroups (range 0.053), with no subgroup falling below 0.89. This near-uniform behavior is consistent with the Static-BDT anchoring rule being a global probability-blending operation that does not condition on respondent characteristics; the empirical frontier itself is population-averaged by construction, and the resulting anchoring benefit is therefore expected to transfer across subgroups defined by age, gender, and education within this sample.

These subgroup results should not be over-interpreted. The 813 \emph{alternative-level} rows subset (from 205 held-out respondents) does not provide sufficient statistical power to detect small subgroup-specific effects, and the Wuhan sample is not designed to be demographically representative of any specific target population. The subgroup analysis is presented as a robustness check on the homogeneity of the anchoring effect, not as evidence for the generalizability of the empirical frontier itself. Future work with larger and more diverse samples is required to assess whether the framework's anchoring benefit is preserved across demographic groups in other populations.
...
\section{Task-Level Multinomial Held-Out Validation}
\label{app:multinom_validation}

The 813 alternative-level rows reported in Section~4.2 of the main text use the alt-level binary log loss $\ell(y, p) = -y \log p - (1-y) \log (1-p)$, where each row is one of three alternatives within one of six choice tasks per respondent. A natural reviewer question is whether the alt-level binary loss is the right proper score for a three-alternative choice task. The binary score implicitly treats each alternative as an independent binary outcome, whereas the genuine generative model is a three-alternative categorical choice in which $P(A) + P(B) + P(C) = 1$ within each task. We address this directly with a complementary task-level multinomial held-out validation.

\subsection{Setup}

We hold out 20\% of respondents (205 of 1,027, seed = 2026) and refit the corrected 6-parameter conditional logit on the remaining 80\% (822 respondents). The three methods being compared differ in how the task-level probability $\Pr(i \mid \text{task})$ is constructed for $i \in \{A, B, C\}$, and we now make the per-alternative and per-task constructions explicit:

\emph{Method 1 (Pure-DCE).} For each test respondent task, the conditional logit supplies utility $U_{A}$, $U_{B}$, $U_{C}$. The task-level multinomial probabilities are computed by the strict conditional-logit softmax
\[
\Pr_{\text{DCE}}(i \mid \text{task}) = \frac{\exp\!\bigl[U_i\bigr]}{\sum_{j \in \{A, B, C\}} \exp\!\bigl[U_j\bigr]},
\]
Taking the opt-out alternative as the utility reference, subtracting its utility from all three alternatives yields utilities $U_A - \hat\beta_{\mathrm{ASC}}$, $U_B - \hat\beta_{\mathrm{ASC}}$, and $0$; this common normalization leaves softmax probabilities unchanged.

\emph{Method 2 (Unconstrained LLM).} For each test respondent task, the matched (wait, eff, se) cell supplies a per-alternative LLM acceptance probability $p_{\text{LLM}, i} \in [0, 1]$ (averaged over 10 repeated LLM calls in the unconstrained Qwen2.5-72B-Instruct prompting condition). The LLM provides binary acceptance probabilities for A and B (and a placeholder probability for C, which is $\Pr(C) = 0$ for the unconstrained condition because the prompt elicits a yes/no decision against the offered policy). The task-level probability is the renormalized binary blend
\[
\Pr_{\text{LLM}}(i \mid \text{task}) = \frac{p_{\text{LLM}, i} \cdot \mathbf{1}[i \in \{A, B\}]}{\sum_{j \in \{A, B\}} p_{\text{LLM}, j}},
\]
which is a softmax over the two non-opt-out alternatives only; this matches how the held-out log-loss code consumes the LLM's binary outputs.

\emph{Method 3 (Static-BDT / EFR).} The Static-BDT prediction per alternative is constructed as a per-alternative binary acceptance probability blend
\[
p_{\text{BDT}, i} = \lambda \cdot p_{\text{LLM}, i} + (1 - \lambda) \cdot p_{\text{DCE}, i}, \quad \lambda = 0.25,
\]
where $p_{\text{DCE}, i} = \sigma\!\bigl[U_i - \hat\beta_{\mathrm{ASC}}\bigr]$ is the same empirical-frontier binary probability used in the alt-level binary validation. Because $p_{\text{BDT}, i}$ is a binary acceptance probability rather than a softmax weight, the task-level probability is the renormalized blend
\[
\Pr_{\text{BDT}}(i \mid \text{task}) = \frac{p_{\text{BDT}, i} \cdot \mathbf{1}[i \in \{A, B\}]}{\sum_{j \in \{A, B\}} p_{\text{BDT}, j}},
\]
i.e. the convex blend is taken per alternative in the alt-level binary space, then the task-level score is obtained by normalizing the two non-opt-out alternatives to sum to 1. This is the construction that the held-out code implements for both the 813 alt-level rows and the 78 task-level matched tasks; we report it explicitly here so the reviewer does not need to guess.

The LLM lookup is restricted to the 11 (wait, eff, se) cells in the 12-cell candidate superset (wait $\in \{0, 6\}$, eff $\in \{0.5, 0.7\}$, se $\in \{1, 2, 3\}$) that are actually populated in the matched subset (the full 12-cell candidate superset is the literal Cartesian overlap on the three focal attributes between the intended DCE non-optout wait/eff/se support and the 64-state grid; 1 of these 12 cells, namely $(wait=0, eff=0.7, se=2)$, is not observed in the matched non-optout pool; 11 cells remain). The held-out pool contains 3,690 alt-rows; after excluding one anomalous wait=2 alt-row, 3,689 rows remain under the intended design, of which 813 are matched and 2,876 are unmatched. Only the 813 matched alt-rows are used for the alt-level binary validation; unmatched tasks are excluded from the LLM and Static-BDT comparison.

\subsection{Two-Alternative Subset (A versus B, opt-out excluded)}

Restricting to test tasks in which the chosen alternative is A or B (excluding opt-out chosen), and restricting to the LLM-matched subset of the held-out 20\% respondent split under the canonical 11-of-12-cell populated intersection ($n_{\text{tasks, matched}} = 78$), the task-level multinomial log loss evaluated on the \emph{same} $n=78$ matched tasks for all three methods is:

\begin{table}[h]
\centering
\caption{Task-level multinomial held-out log loss on the 2-alternative subset (A vs. B, opt-out excluded), evaluated on the LLM-matched subset of the held-out 20\% respondent split (seed 2026, intended design, exclude wait=2 outlier). All three methods are evaluated on the same $n=78$ matched tasks. Lower is better. The conditional logit is refit on the 80\% training split (six parameters: vaccine origin, wait, efficacy, side effects, cash, opt-out ASC).}
\label{tab:multinom_2alt}
\footnotesize
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.25cm} >{\centering\arraybackslash}p{1.45cm} >{\centering\arraybackslash}p{1.45cm} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Method} & \textbf{Log Loss} & \textbf{N (tasks)} & \textbf{Note} \\
\midrule
Pure-DCE & 0.3307 & 78 & Refit 6-param conditional logit, $\Pr(A)+\Pr(B)=1-\Pr(C)$ restricted to $\{A,B\}$ \\
Unconstrained LLM & 0.8521 & 78 & Direct LLM $\Pr(A)$ and $\Pr(B)=1-\Pr(A)$, restricted to $\{A,B\}$ \\
Static-BDT ($\lambda=0.25$) & \textbf{0.4307} & 78 & Convex blend $0.25\cdot\pi_{\text{LLM}}+0.75\cdot\pi_{\text{DCE}}$, restricted to $\{A,B\}$ \\
\bottomrule
\end{tabularx}
\end{table}

On this apples-to-apples matched subset, Static-BDT ($0.4307$) sits \emph{between} the two baselines, reducing task-level log loss by $49.5\%$ relative to the unconstrained LLM ($0.8521$) but remaining $30.2\%$ above the empirical-anchor Pure-DCE frontier ($0.3307$): the small blend weight $0.25$ is sufficient to absorb the LLM's worst monotonicity violations, but the LLM component still pulls predictions away from the empirical frontier in the task-level proper score. The two proper scores are not directly comparable in absolute terms---the alt-level binary score treats the unmatched alternatives as missing not at random (i.e., only the matched alternatives within a 2-alternative non-opt-out task contribute a loss term, and the loss is averaged over those rows), while the task-level multinomial score renormalises the two non-opt-out alternatives to sum to 1 and computes the joint log loss over the (A, B) choice. Both proper scores are unbounded above, with the uniform-prediction reference giving a baseline of $\log 2 \approx 0.693$ for the binary case and a comparable baseline for the two-alternative multinomial; we report all three numerical values (binary $0.6789$ / $0.6985$ / $0.8273$ and task-level $0.3307$ / $0.4307$ / $0.8521$) directly without any rescaling, so that the absolute magnitudes can be read against the same uniform-prediction baseline.

\subsection{Three-Alternative Full Tasks (A, B, C)}

The strict 11-of-12-cell populated intersection has no opt-out alternative in its support, because the opt-out alternative is by construction at $(w,e,s) = (0,0,0)$ and the grid cells all have $e \in \{0.5, 0.7\} \neq 0$. Consequently the three-alternative full-task subset, which requires all three alternatives (A, B, C) to lie in the LLM-matched cell set, is empty under the canonical match ($n_{\text{tasks, matched}} = 0$ for the 3-alt full subset; $n_{\text{AB chosen, matched}} = 78$ for the 2-alt subset, which is the same set as above). A coarser match that admits the opt-out placeholder cell by relaxing the efficacy constraint to $e \in \{0, 0.5, 0.7\}$ would recover a non-empty 3-alt full subset, but that relaxation would also admit the opt-out placeholder's $(0,0,0)$ which carries no real attribute information, and so we report the canonical 2-alt subset as the primary apples-to-apples task-level score.

\begin{table}[h]
\centering
\caption{Three-alternative full task subset under the canonical 11-of-12-cell match is empty by construction (the opt-out alt at $(0,0,0)$ has $e = 0$ and never lies in the 12-cell candidate superset). We report the 2-alt subset from Table~\ref{tab:multinom_2alt} as the primary task-level score, with the LLM probability for C drawn from the refit 6-param conditional logit for completeness. The 2-alt subset excludes the held-out tasks in which the opt-out was chosen, which are dominated by the unconditional $\Pr(C) \approx 0.20$ from the empirical frontier and would inflate the proper score by a uniform $-\log 0.20 \approx 1.61$ contribution that does not distinguish methods.}
\label{tab:multinom_3alt}
\footnotesize
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{4.15cm} >{\centering\arraybackslash}p{1.45cm} >{\centering\arraybackslash}p{1.45cm} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Method} & \textbf{Log Loss} & \textbf{N (tasks)} & \textbf{Note} \\
\midrule
Pure-DCE (3-alt softmax; empty under canonical match) & --- & 0 & No full-3-alt task satisfies the 11-of-12-cell canonical match \\
Unconstrained LLM (2-alt only) & 0.8521 & 78 & $\Pr(A)$ and $\Pr(B)=1-\Pr(A)$; same 2-alt subset as Table~\ref{tab:multinom_2alt} \\
Static-BDT ($\lambda=0.25$, 3-alt; empty under canonical match) & --- & 0 & Same as Pure-DCE: 0 full-3-alt tasks under canonical match \\
\bottomrule
\end{tabularx}
\end{table}

\noindent Under the canonical 11-of-12-cell populated intersection the 3-alt full subset is empty, so the apples-to-apples task-level score is the 78-task 2-alt subset from Table~\ref{tab:multinom_2alt}.

\subsection{Implication for the 64-State Validation}

The 64-state grid evaluation in the main text uses a single probability per (wait, eff, se) cell, with cash and vaccine-origin attributes held at their reference values (cash = 0, origin = 0). The held-out DCE tasks administer cash $\in \{50, 200, 800\}$ and origin $\in \{0, 1\}$, so the LLM lookup matches $78$ of the $1{,}236$ test tasks ($6.3\%$) for the 2-alternative subset under the canonical 11-of-12-cell populated intersection. For these matched tasks, the task-level multinomial validation gives a more nuanced picture than the alt-level binary pattern: Static-BDT ($\lambda = 0.25$) reduces task-level log loss by $49.5\%$ relative to the unconstrained LLM ($0.4307$ vs. $0.8521$) but remains $30.2\%$ above the empirical-anchor Pure-DCE frontier ($0.3307$). On the alt-level binary $813$-row matched subsample (a different aggregation that retains the 2-alts-per-row structure used in the main text), Static-BDT is essentially tied with Pure-DCE ($0.6985$ vs. $0.6789$); the residual $2.9\%$ gap is the same direction as in the task-level score and consistent with a small blend weight. For the unmatched test tasks, the frontier-only Pure-DCE prediction remains the only validated method, and the LLM and Static-BDT entries are not directly comparable because the LLM is not administered at the held-out (wait, eff, se) cell with non-reference cash and origin values.
