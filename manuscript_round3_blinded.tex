...
\caption{Exact-task categorical held-out validation multinomial log loss (Pure-DCE: $0.911$; EFR: $0.923$; Raw LLM: $1.020$; uniform benchmark: $1.099$) on six canonical DCE tasks ($N=205$ held-out respondents, 1,230 choices). EFR materially improved exact-task predictive performance relative to the unconstrained LLM ($9.5\%$ reduction in log loss) and approached the predictive performance of the Pure-DCE model (within $1.3\%$), while retaining a non-zero LLM component for narrative extension.}
