...
\small
\textit{Note.} Uniform $p$ = fraction of null replicates $\geq \rho_{\text{EFR}}$ under the uniform-noise null. Permutation $p$ = same under the permutation null. The permutation null is the stricter test: it preserves the marginal distribution of the actual LLM output and destroys only the state-level association. Qwen2.5-72B does not exceed the permutation null ($p = .179$), indicating that its rank information is largely a mechanical consequence of the 75\% $P_{\text{emp}}$ anchor; the LLM component provides negligible additional rank signal. DeepSeek V4 and MiroThinker exceed both nulls, indicating that their LLM components contribute genuine rank information beyond mechanical anchoring. All observed $\rho$ values match Table~1 of the main text to display precision.
\end{minipage}
\end{table}

The reported frontier-alignment $\rho$ values in Table~1 of the main text and Table~\ref{tab:lambda_frontier} of this supplement should be read as structural diagnostics of how closely the EFR blend tracks the empirical frontier; the uniform null confirms that all three models exceed pure noise, while the permutation null distinguishes models whose LLM component contributes genuine rank information (DeepSeek V4, MiroThinker; $p \leq .003$) from those where the rank signal is primarily mechanical (Qwen2.5-72B; $p = .179$). The OOD frontier-alignment values ($\rho = 0.87$--$0.93$ for Qwen; $\rho = 0.87$--$0.88$ for DeepSeek V4) sit inside one SD of the tied Panel B null (mean $\rho = 0.89$, SD $= 0.06$) and should be read as descriptive structural robustness measures. The fairer predictive benchmark---held-out DCE log loss---is reported in Section~4.2 of the main text.
