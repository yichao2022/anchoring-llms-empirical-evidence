...
\end{itemize}
The bootstrap resamples 206 respondents with replacement (2,000 replications), maintaining the within-respondent cluster structure (6 choices per respondent). The 95\% confidence intervals quantify the uncertainty in these paired differences.

\begin{table}[h]
\centering
\caption{Respondent-level paired cluster bootstrap for exact-task multinomial log loss ($N = 206$ respondents, 1,236 choices; 2,000 replications).}
...
\midrule
EFR vs. Raw LLM ($\Delta LL_{\text{EFR-Raw}}$) & $-0.0967$ & [TBD] \\
EFR vs. Pure-DCE ($\Delta LL_{\text{EFR-Pure}}$) & $+0.0121$ & [TBD] \\
\bottomrule
\end{tabular}
\end{table}
...
All 1,027 respondents completed the same six choice tasks, each presenting two non-opt-out alternatives (A, B) and an opt-out alternative (C). The six administered tasks comprised 12 unique non-opt-out profiles, covering 12 of 216 candidate profile cells (5.6\%). We hold out 20\% of respondents ($N = 206$) using a seeded random split (seed = 2026; 821 training, 206 test) and use their choices on the six canonical tasks for validation. Exact log loss values will be updated following the canonical seeded split implementation; preliminary estimates are reported in Table~\ref{tab:multinom_6canonical}.
...
\item \textbf{EFR ($\lambda = 0.25$):} Convex blend $\pi_{\text{EFR}} = 0.25 \cdot \pi_{\text{LLM}} + 0.75 \cdot P_{\text{emp}}$.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Primary exact-task multinomial held-out log loss on six canonical DCE tasks ($N = 206$ respondents, 1,236 choices). Lower is better. The conditional logit is refit on the 80\% training split. LLM predictions generated via local Ollama Qwen2.5-72B (temperature 0.7, 10 repetitions per task).}
\label{tab:multinom_6canonical}
\footnotesize
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.15}
...
\item \textbf{All models vs. Uniform:} All three model-based approaches outperform the equal-probability benchmark (log 3 $\approx$ 1.099), confirming behavioral signal in both the DCE and LLM components.
\end{itemize}
