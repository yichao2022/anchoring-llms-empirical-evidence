...
\title{Online Supplement: Anchoring Large Language Models to Empirical Behavioral Evidence: A Hybrid Approach for DCE-Grounded Narrative Policy Simulation}
...
To interpret the frontier-alignment Spearman $\rho(\pi_{\text{EFR}}, P_{\text{emp}})$ reported in Table~1 of the main text and Table~\ref{tab:S6} of this supplement, we generated two Monte Carlo null distributions on the 64-state grid (10{,}000 replicates each).

\paragraph{Uniform-noise null.} The LLM component is replaced with a uniform random draw $U[0, 1]$ on each state, then $\rho(0.25 \cdot U + 0.75 \cdot P_{\text{emp}},\ P_{\text{emp}})$ is computed. This answers: ``If the LLM component were pure uniform noise, how much rank correlation would the 75\% anchor alone produce?'' The resulting null has mean $\rho = 0.366$ (SD $= 0.106$; 95\% CI $[0.146,\,0.561]$).

\paragraph{Permutation null.} The actual LLM probability vector $\{p_{\text{LLM},1}, \ldots, p_{\text{LLM},64}\}$ is permuted across grid states (preserving the marginal distribution of the LLM output), then $\rho(0.25 \cdot \pi(p_{\text{LLM}}) + 0.75 \cdot P_{\text{emp}},\ P_{\text{emp}})$ is computed. This answers: ``Does the real LLM provide rank information beyond what mechanical anchoring from its own marginal distribution would produce?'' This is the stricter benchmark because it preserves the variance and central tendency of the actual LLM output. The canonical $P_{\text{emp}}$ is the 6-parameter conditional-logit frontier (Appendix~D); the LLM predictions for DeepSeek V4 Pro and MiroThinker-1.7-mini are from the same API deployments used in Table~1, while the Qwen2.5-72B prediction is from a local Ollama replication (quantized Q4\_K\_M; differs from the Hugging Face inference endpoint used in Table~1 but reproduces the same canonical $\rho = 0.689$ under the 6-param frontier).

\paragraph{Results.} Table~\ref{tab:S1} compares the observed EFR $\rho$ against both nulls.

\begin{table}[h]
\centering
\caption{Observed EFR Spearman $\rho$ versus uniform-noise and permutation nulls on the 64-state grid ($\lambda = 0.25$; 10{,}000 null replicates).}
\label{tab:S1}
\small
\begin{tabular}{lcccc}
\toprule
Model & $\rho_{\text{EFR}}$ & Uniform $p$ & Permutation $p$ & Permutation 95\% CI \\
\midrule
Qwen2.5-72B-Instruct & 0.689 & $<.001$ & .179 & $[0.50, 0.75]$ \\
DeepSeek V4 Pro & 0.812 & $<.001$ & .003 & $[0.54, 0.78]$ \\
MiroThinker-1.7-mini & 0.756 & $<.001$ & $<.001$ & $[0.17, 0.58]$ \\
\bottomrule
\end{tabular}
\vspace{0.3em}
\begin{minipage}{0.92\textwidth}
\small
\textit{Note.} Uniform $p$ = fraction of null replicates $\geq \rho_{\text{EFR}}$ under the uniform-noise null. Permutation $p$ = same under the permutation null. The permutation null is the stricter test: it preserves the marginal distribution of the actual LLM output and destroys only the state-level association. Qwen2.5-72B did not show detectable incremental DCE-aligned rank information beyond the distribution-preserving permutation benchmark ($p = .179$). DeepSeek V4 Pro and MiroThinker showed incremental DCE-aligned rank information beyond mechanical anchoring ($p = .003$ and $p < .001$, respectively). After Bonferroni correction for three models ($\alpha = .05/3 = .017$), both DeepSeek V4 Pro and MiroThinker remain significant. All observed $\rho$ values match Table~1 of the main text to display precision.
\end{minipage}
\end{table}

The reported frontier-alignment $\rho$ values in Table~1 of the main text and Table~\ref{tab:S6} of this supplement should be read as structural diagnostics of how closely the EFR blend tracks the empirical frontier; the uniform null confirms that all three models exceed pure noise, while the permutation null distinguishes models whose LLM component shows detectable DCE-aligned rank information beyond mechanical anchoring (DeepSeek V4 Pro, MiroThinker; $p \leq .003$) from those that do not (Qwen2.5-72B; $p = .179$). The OOD frontier-alignment values ($\rho = 0.87$--$0.93$ for Qwen; $\rho = 0.87$--$0.88$ for DeepSeek V4 Pro) sit inside one SD of the tied Panel B null (mean $\rho = 0.89$, SD $= 0.06$) and should be read as descriptive structural robustness measures. The fairer predictive benchmark---held-out DCE log loss---is reported in Section~4.2 of the main text.
...
\midrule
Qwen2.5-72B-Instruct       & Local vLLM-MLX (Apple M4 Max)         & 2026-05-19            & 0.7    & 1.0 & 512    & 10 & Full-scale primary; Main Table 1; Tables S5--S6; same run as Table S10 \\
DeepSeek V4 Pro                & DeepSeek API                          & 2026-05-19            & 0.7    & 1.0 & 512    & 10 & Full-scale robustness; Main Table 1; Table S6 \\
MiroThinker-1.7-mini       & Local vLLM-MLX (Apple M4 Max)         & 2026-05-19            & 0.0    & 1.0 & 256    & 10 & Full-scale robustness (deterministic); Main Table 1; Table S6; same run as Table S10 \\
\midrule
...
The cash-incentive attribute is held at $0$ RMB in the 64-state grid (the LLM and EFR comparison absorbs the cash coefficient into the opt-out alternative-specific constant); the vaccine-origin attribute is held at the reference (domestic) framing. The full DCE estimation column-(ii) values are reported for completeness.
...
\textbf{Preprocessing rule.} One anomalous alternative-level row (RespondentID~1, task~1, wait~$=2$ months, a level not administered in the DCE design matrix) is excluded from the raw coded data before \emph{all} estimation, before the 64-state frontier fit, and before the held-out respondent split; no analysis reported in this paper uses the wait$=2$ row. The single explicit exclusion is applied uniformly across the full-sample frontier coefficients (Table~\ref{tab:S5}), the data-scaling ablation (Table~\ref{tab:S8}), and the held-out DCE predictive validation (Table~\ref{tab:S7}).

Table~\ref{tab:S5} reports the static conditional-logit estimates used to construct the empirical choice frontier. The signs follow the expected behavioral directions: waiting time and side effects reduce acceptance, while vaccine efficacy and cash incentives increase acceptance.
...
All coefficients display the expected sign. The negative coefficient on Wait Time implies that longer waiting time lowers the empirical probability of vaccine acceptance (robust negative signal: $\hat\beta_{\text{wait}} = -0.059$, 95\% CI $[-0.076, -0.043]$), while the negative coefficient on SideEffects implies that greater side-effect burden lowers acceptance, though this effect is weaker and the confidence interval includes zero (exploratory negative signal: $\hat\beta_{\text{SE}} = -0.036$, 95\% CI $[-0.085, 0.014]$). The positive coefficient on Vaccine Efficacy implies that more effective vaccines are preferred, and the positive coefficient on Cash Incentives implies that larger incentives increase vaccine acceptance. The negative ASC coefficient ($\hat\beta_{\mathrm{ASC}} = -0.2100$, with the ASC coded as $+1$ for the opt-out alternative and $0$ for the non-opt-out alternatives during estimation) means that, holding the policy attributes fixed, respondents are \emph{more} likely to choose the policy than to opt out, not less: under $P_{\mathrm{emp}} = \sigma[U(x, \bar\beta) - \hat\beta_{\mathrm{ASC}}]$, the negative ASC enters the sigmoid as a positive additive constant to the utility gap, so $P_{\mathrm{emp}} > \sigma[U(x, \bar\beta)]$ for any non-opt-out policy $x$, and the probability mass on the opt-out shrinks. Equivalently, $\exp(\hat\beta_{\mathrm{ASC}}) = 0.61 < 1$ indicates that the odds of selecting the opt-out are 0.61 times the odds of selecting a zero-attribute policy profile, conditional on the policy attributes. The waiting-time directional restriction defines an \textbf{empirically grounded monotonicity condition} (MVR-Wait), while the side-effect restriction is an \textbf{expected directional relationship} with weaker empirical support (MVR-SE is reported for structural completeness).

\subsection{Connection to the 64-State Evaluation Grid}

The 64-state grid used in the full-scale EFR experiments is not a respondent-level train--test split. Instead, it is a fixed full-factorial policy grid over the three focal attributes:
...
The full-scale LLM and EFR results in Table~1 of the main text are evaluated by comparing agent-implied probabilities against \(P_{\text{static},g}\) over this common 64-state grid. This design ensures that all model comparisons are made on the same policy-relevant attribute space.
...
\subsubsection{Rule-Injection EFR Prompt}

The rule-injection EFR prompt added an explicit monotonicity rule before the same decision scenario:
...
\subsection{Relationship to the Full-Scale EFR}

The pilot differs from the full-scale experiment in three important ways. First, the pilot used a smaller 1.5B local model, whereas the full-scale experiment uses Qwen2.5-72B-Instruct as the primary model and includes DeepSeek V4 Pro and MiroThinker-1.7-mini as robustness models. Second, the pilot varied only waiting time while holding cost and side-effect risk fixed, whereas the full-scale experiment evaluates a 64-state grid over waiting time, vaccine efficacy, and side-effect burden. Third, the pilot used prompt-level rule injection, whereas the full-scale EFR applies a post-generation convex anchoring rule:

\[
\pi_{\text{EFR}}(y=1 \mid x)
=
...
The pilot therefore provides low-resource mechanism evidence that monotonicity information can reduce frontier-relative behavioral inconsistencies, while the full-scale experiment evaluates the main EFR method: inference-time frontier-anchored regularization against the empirical choice frontier.


\section{EFR and \texorpdfstring{\(\lambda\)}{lambda} Sensitivity}
\label{app:lambda}
...
The EFR is computed as a post-generation convex combination:

\[
\pi_{\mathrm{EFR}}(y=1 \mid x_g)
=
...
This distinction is important because the Pure-DCE endpoint \((\lambda=0)\) mechanically achieves perfect frontier alignment when evaluated against \(P_{\mathrm{static}}\). The purpose of EFR is not to outperform Pure-DCE in frontier-deviation metrics; rather, it is to regularize LLM-generated probabilities toward the empirical frontier while preserving an explicit LLM component. The sensitivity analyses below therefore report both frontier-alignment diagnostics and held-out DCE predictive metrics.

\subsection{Frontier-Alignment Sensitivity}

Table~\ref{tab:S6} reports frontier-alignment sensitivity on the 64-state policy grid. Metrics are evaluated against the full-sample empirical frontier \(P_{\mathrm{static}}\). As expected, lower values of \(\lambda\) produce stronger alignment with the empirical frontier because the anchored probability places more weight on \(P_{\mathrm{static}}\). The Pure-DCE endpoint \((\lambda=0)\) has zero frontier-deviation by construction, while the unconstrained LLM endpoint \((\lambda=1)\) has the largest MSE, MAE, and monotonicity violation rates.
...
The pattern in Table~\ref{tab:S6} confirms that the alignment gains reported in the main results are driven by empirical-frontier anchoring rather than by the unconstrained LLM alone. As \(\lambda\) increases, MSE and MAE rise, MVR-Wait and MVR-SE increase, and Spearman rank alignment declines. This is the expected behavior of a regularization layer that interpolates between a structured empirical benchmark and an unconstrained generative model.

\subsection{Held-Out Predictive Sensitivity}
\label{app:lambda_heldout_sensitivity}

Table~\ref{tab:S7} (this supplement) reports a partial-support grid-overlap diagnostic on the LLM-matched held-out subset. In this diagnostic, the empirical frontier is estimated on training respondents and evaluated on held-out DCE choices. The matched subset contains \(N=813\) \emph{alternative-level} binary-outcome rows whose attribute profiles can be linked to available LLM grid probabilities on three focal attributes---waiting time, efficacy, and side-effect burden---only; cash incentive and vaccine origin are not matched between DCE alternatives and the LLM grid (each row is one of three alternatives within one of six tasks). Log loss and Brier score are reported for completeness; AUC is retained only as a secondary ranking diagnostic and should be interpreted cautiously. \textbf{This is not a full-profile predictive validation;} the primary held-out predictive validation is the exact-task multinomial log loss reported in Table~S16.

\begin{table}[htbp]
\centering
\caption{$\lambda$-sensitivity for held-out DCE predictive validation on the LLM-matched held-out subset ($N=813$ \emph{alternative-level binary-outcome rows} from $N=205$ held-out respondents (each row is one of three alternatives within one of six respondent-specific task draws; the held-out pool contains $N=3{,}690$ alt-rows; after excluding one anomalous wait=2 alt-row, 3{,}689 rows remain under the intended design, of which 813 are matched and 2{,}876 are unmatched)). This is a \textbf{secondary sensitivity analysis}; the primary predictive validation is the exact-task multinomial log loss (Table~\ref{tab:S16}). Log loss and Brier score are reported for comparison; AUC is included only as a ranking diagnostic. AUC values are $\mathrm{AUC}(y, p_\mathrm{BDT})$ with $y=1$ = acceptance; see the orientation diagnostic below for an explanation of why $\mathrm{AUC}(y, 1-p)$ values were previously reported in this column. \emph{MSE and MAE decrease trivially with $\lambda$ because they are algebraically induced by the convex blend $\pi_{\mathrm{EFR}} = \lambda \bar{\pi}_{\mathrm{LLM}} + (1-\lambda) P_{\mathrm{emp}}$; only Spearman's $\rho$ against the frontier is a non-trivial alignment diagnostic and it is reported in the main text.}}
\label{tab:S7}
...
For each training share \(s\), the EFR is computed using the same unconstrained Qwen2.5-72B-Instruct probabilities \(\bar{\pi}_{\mathrm{LLM}}\) reused from the full-scale experiment. The anchored probability for state \(x_g\) is:
\[
\pi_{\mathrm{EFR},s}(y=1 \mid x_g)
=
...
Two patterns emerge. Against the full-sample canonical empirical frontier, all three metrics improve as more DCE respondents are used to estimate the empirical frontier. The sharp improvement at $100\%$ reflects the transition from a subsample-reestimated frontier to the full-sample canonical frontier, indicating that EFR's precision is partly gated by the precision of the empirical frontier. Against each subsample's own reestimated frontier, performance is stable across the $30\%$--$70\%$ subsamples, indicating that the anchoring mechanism stays close to whichever frontier it is given, even when that frontier is estimated from limited data; the 100\% row coincides in both blocks because the full-sample fit is the same.

\subsection{Interpretation}
\label{app:interpretation}

Taken together, the two benchmark blocks in Table~\ref{tab:S8} separate two sources of improvement in the EFR framework.
...
The practical implication is that EFR may remain useful with partial DCE data, because even smaller empirical frontiers can impose meaningful directional discipline on LLM-generated choices. The main benefit of larger samples is improved frontier precision, which translates into lower MSE relative to the best available empirical benchmark. This property makes EFR potentially useful in settings where large-scale DCE data collection is infeasible, while also providing a rationale for investing in larger samples when high-precision policy simulation is required.
...
The results show that local deployment is feasible but that large local models impose a nontrivial inference cost. The Qwen2.5-72B-Instruct local run required approximately 11.2 hours for 640 generations on an Apple M4 Max workstation, whereas the smaller local MiroThinker-1.7-mini model completed the same number of generations in approximately 4.2 minutes. The EFR anchoring rule itself is computationally lightweight: once raw LLM probabilities and empirical frontier values are available, the post-generation correction requires only a convex probability-level calculation.

\section{Pure-DCE Reference and Policy-Ranking Validation}\label{sec:heldout_validation}
\label{app:pure_dce_prc}

This appendix documents the Pure-DCE reference and the descriptive Policy Ranking Consistency (PRC) calculation reported in Table~\ref{tab:S11}. The purpose is to make the policy-ranking exercise reproducible by specifying the policy set, eligible grid states, policy-effect calculation, and pairwise ranking rule. \emph{PRC is reported as a structural diagnostic of the 75\% $P_{\text{emp}}$ blend, not as an independent behavioral validation; no significance test is performed because the 15 unordered policy pairs share six underlying policy effects and are not 15 independent Bernoulli trials.}
...
\midrule
DeepSeek V4 Pro & Unconstrained LLM & 2/15 & 0.133 \\
DeepSeek V4 Pro & EFR & 9/15 & 0.600 \\
\midrule
...
\]
It contains no LLM-generated component. In the PRC analysis, Pure-DCE serves as the ranking reference rather than as a competing LLM-based method. Its own PRC relative to itself is therefore mechanically equal to one and is not reported as a separate row in Table~\ref{tab:S11}.
...
\}.
\]
EFR probabilities are computed using the main anchoring specification:
\[
\pi_{\mathrm{EFR}}
=
...
The PRC computation is conducted on the same fixed 64-state grid used in the main frontier-alignment analysis. For each model, the analysis verifies that all 64 states are present, that the Pure-DCE probability equals \(P_{\mathrm{static}}\), and that the EFR probability satisfies
\[
\pi_{\mathrm{EFR}}
=
0.25\,\bar{\pi}_{\mathrm{LLM}}
...
0.75\,P_{\mathrm{static}}
\]
up to numerical tolerance. The resulting PRC values are reported in Table~\ref{tab:S11}. The metric should be interpreted as a structural diagnostic of the 75\% $P_{\text{emp}}$ blend, not as external validation against realized vaccine uptake and not as a hypothesis test.
...
For \textbf{held-out prediction metrics} (Figure~1 of the main manuscript reports the primary exact-task multinomial validation; respondent-level paired cluster bootstrap for the paired log loss differences is reported below), the DCE data possess a nested panel structure where multiple choice observations are clustered within individual respondents. To strictly account for within-respondent correlation and avoid underestimating standard errors, we employ a \textbf{respondent-level cluster bootstrap}. Specifically, we resample the 205 unique respondents (whose choices constitute the 1,230 task-level multinomial observations) with replacement, maintaining the intact within-respondent choice structure. We perform 2,000 replications to ensure stable interval estimation.

For \textbf{frontier-deviation metrics} (Table~\ref{tab:S13}), the evaluation operates on the fixed 64-state policy grid rather than respondent-level choices. Therefore, we compute frontier-deviation intervals by resampling the 64 policy-grid states with replacement using 2,000 replications. For \textbf{monotonicity violation rates} (MVR), we resample comparable attribute pairs. For \textbf{policy-ranking consistency} (Table~\ref{tab:S11} in this supplement, discussed in the main text Policy Ranking Consistency section), we report no confidence intervals and no significance test, because the 15 unordered policy pairs share six underlying policy effects and are not 15 independent Bernoulli trials; the metric is reported as a descriptive share only.

These intervals provide uncertainty summaries for the finite evaluation objects used in the paper: policy-grid states, held-out matched respondents, monotonicity-comparison pairs, and policy-pair rankings. They are not a substitute for external validation against realized uptake or for respondent-level uncertainty from re-estimating the full 6-parameter conditional-logit DCE frontier. The paired bootstrap intervals condition on the fitted training-split DCE model and the realized averaged LLM task probabilities; they quantify held-out respondent sampling uncertainty and do not incorporate DCE-estimation or LLM-generation uncertainty.

\subsection{Held-Out Prediction Uncertainty}

Table~\ref{tab:S12} reports respondent-level paired cluster bootstrap confidence intervals for the primary exact-task multinomial validation. The paired differences are:
\begin{itemize}
...
Table~\ref{tab:S13} reports bootstrap intervals for the primary frontier-alignment diagnostics discussed in the main text. The intervals support the main Qwen2.5-72B-Instruct alignment result: the MSE and Spearman $\rho$ intervals do not overlap between EFR and the unconstrained LLM. The MVR-Wait point estimates for Qwen2.5-72B-Instruct also do not overlap (raw $0.6250$, EFR $0.3021$); the bootstrap intervals, however, are wide and overlap on the 64-state grid (state-level resampling leaves the 16 (eff, se) cell pattern close to identical across replicates), so the MVR-Wait improvement is best read from the point estimates and the cross-model ranking rather than from a within-Qwen significance test. The side-effect monotonicity diagnostic is weaker: for MiroThinker-1.7-mini, EFR and the unconstrained LLM produce identical MVR-SE point estimates ($0.3333$) and identical bootstrap CIs $[0.240, 0.427]$, indicating that the EFR anchor at $\lambda=0.25$ does not transmit a sufficient side-effect signal on this frontier.
...
Qwen2.5-72B-Instruct & EFR & MSE & 0.0016 & [0.0011, 0.0022] \\
\midrule
Qwen2.5-72B-Instruct & Unconstrained LLM & MVR-Wait & 0.6250 & point \\
Qwen2.5-72B-Instruct & EFR & MVR-Wait & 0.3021 & point \\
\midrule
Qwen2.5-72B-Instruct & Unconstrained LLM & Spearman \(\rho\) & $0.0608$ & [$-0.1784$, $0.3042$] \\
Qwen2.5-72B-Instruct & EFR & Spearman \(\rho\) & $0.6886$ & [$0.5266$, $0.8012$] \\
\midrule
DeepSeek V4 Pro & Unconstrained LLM & MSE & 0.0379 & [0.0266, 0.0505] \\
DeepSeek V4 Pro & EFR & MSE & 0.0024 & [0.0017, 0.0031] \\
\midrule
DeepSeek V4 Pro & Unconstrained LLM & MVR-Wait & 0.3438 & point \\
DeepSeek V4 Pro & EFR & MVR-Wait & 0.0729 & point \\
\midrule
DeepSeek V4 Pro & Unconstrained LLM & Spearman \(\rho\) & $0.3025$ & [$0.0714$, $0.5123$] \\
DeepSeek V4 Pro & EFR & Spearman \(\rho\) & $0.8123$ & [$0.6832$, $0.8885$] \\
\midrule
MiroThinker-1.7-mini & Unconstrained LLM & MSE & 0.0872 & [0.0662, 0.1107] \\
MiroThinker-1.7-mini & EFR & MSE & 0.0054 & [0.0041, 0.0070] \\
\midrule
MiroThinker-1.7-mini & Unconstrained LLM & MVR-Wait & 0.1042 & point \\
MiroThinker-1.7-mini & EFR & MVR-Wait & 0.0833 & point \\
\midrule
MiroThinker-1.7-mini & Unconstrained LLM & Spearman \(\rho\) & $0.5306$ & [$0.3070$, $0.6966$] \\
MiroThinker-1.7-mini & EFR & Spearman \(\rho\) & $0.7564$ & [$0.5849$, $0.8860$] \\
\bottomrule
\end{tabular}
...
\begin{minipage}{0.95\textwidth}
\small
\textit{Note.} Unlike the held-out prediction metrics in Table~\ref{tab:S7} (held-out prediction metrics), which employ respondent-level cluster bootstrapping to account for the nested panel structure of DCE choices, the frontier-alignment metrics here are evaluated on the fixed 64-state policy grid. Therefore, frontier-deviation intervals are computed by resampling the 64 policy-grid states with replacement (2{,}000 replications). MVR-Wait is reported as a point estimate: the 16 (eff,se) cell-level structure makes the MVR a step function on the grid, so a state-level bootstrap yields degenerate intervals; a comparable-attribute-pair bootstrap (resampling (eff,se) cells) gives the same point estimate by construction because the frontier-deviation ranking is invariant under cell-level resampling. The MiroThinker-1.7-mini side-effect diagnostic (MVR-SE $= 0.3333$ for both unconstrained and EFR) is omitted from this table: the corrected 6-parameter specification (Section~3 of the main text) makes the side-effect coefficient the smallest empirical signal among the five attributes, and the EFR anchor at $\lambda = 0.25$ does not transmit a sufficient side-effect correction to move MiroThinker's MVR-SE off the unconstrained value; the failure mode is reported here as a supplementary structural diagnostic.
\end{minipage}
\end{table}
...
To assess the representativeness of the 22.0\% matched held-out subset ($N=813$ \emph{alternative-level binary-outcome rows} from $N=205$ held-out respondents (each row is one of three alternatives within one of six respondent-specific task draws; the held-out pool contains $N=3{,}690$ alt-rows; after excluding one anomalous wait=2 alt-row, 3{,}689 rows remain under the intended design, of which 813 are matched and 2{,}876 are unmatched)) used in the partial-support grid-overlap diagnostic (Table~\ref{tab:S7}) of this supplement, we compare its covariate distribution against the unmatched held-out alt-row subset ($N=2{,}876$). Matching is on three focal attributes (waiting time, efficacy, side-effect burden) only; cash incentive and vaccine origin are unmatched between DCE alternatives and the LLM grid. Table~\ref{tab:S15} reports the means and Standardized Mean Differences (SMD) for key DCE attributes and the choice rate.
...
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{2.8cm} >{\raggedright\arraybackslash}X@{}}
\toprule
...
The main text (Section~4.3, Table~2 of the main text) reports the out-of-design Panel A and Panel B narrative-policy evaluation on the 20 paired scenarios (40 narrative descriptions) with MVR-Wait on the 8 wait-varying pairs (Panel A: 6; Panel B: 2), MAD/$\rho$ on the 12 narrative-only pairs, and the two-model comparison (Qwen2.5-72B-Instruct and DeepSeek V4 Pro; MiroThinker-1.7-mini was not administered on the OOD panel because its 64-state grid evaluation was used to validate the EFR anchor as a standalone test, not to run the OOD narrative scenarios). The four-panel diagnostic figure (MVR-Wait / Spearman $\rho$ / subgroup heterogeneity / PRC) that combined the 20 paired OOD scenarios with the 813-row matched-subset subgroup analysis is omitted from this supplement because the rank-correlation panel falls inside the construction-only null band; the table-based diagnostics (Table~2 of the main text) are the authoritative source for the OOD narrative results. The out-of-design evaluation is restricted to the two-model Qwen2.5-72B-Instruct and DeepSeek V4 Pro panel; the third model (MiroThinker-1.7-mini) is reported only on the 64-state grid in the main text.

Full scenario descriptions for all 20 pairs (40 narratives) are provided in Table~\ref{tab:S13_ood_scenarios}. Each scenario specifies the waiting time, vaccine efficacy, and side-effect level used to compute the empirical frontier $P_{\text{emp}}$ per Table~S4, along with the Chinese narrative description presented to the LLM.

\input{table_s13_ood_scenarios}
...
This appendix reports the primary exact-task multinomial held-out validation on the six administered DCE task slots (Choicesets 1--6) completed by all 1,027 respondents. The validation uses a local replication run with Qwen2.5-72B-Instruct via Ollama (temperature 0.7, 10 repetitions per task) to generate LLM predictions for direct comparison with the empirical choice data.

\subsection{Design and Setup}

All 1,027 respondents each completed six randomized choice tasks drawn from the full factorial design, with each task presenting two non-opt-out alternatives (A, B) and an opt-out alternative (C). The administered tasks comprised 12 unique non-opt-out profiles, covering 12 of 216 candidate profile cells (5.6\%). A post-hoc dominance audit of the A-versus-B profile pairs confirmed no strict dominance across the five monotonic attributes; two tasks (Tasks 1 and 3) exhibited near-dominance with one alternative winning on four of five attributes, but each retained at least one attribute favoring the other alternative. We hold out 20\% of respondents ($N = 205$) using a seeded random split (seed = 2026; 822 training, 205 test) and use their choices on the six administered tasks for validation. Exact log loss values are reported in Table~\ref{tab:S16}.
...
\item \textbf{EFR ($\lambda = 0.25$):} Convex blend in the three-alternative probability simplex. For each task $t$, the DCE training-split model yields $\mathbf{p}^{\mathrm{DCE}}_t = \mathrm{softmax}(V_{At}, V_{Bt}, V_{Ct})$ where $V_{Ct} = \hat\beta_{\mathrm{ASC}}$ (the opt-out alternative-specific constant), and the LLM yields $\mathbf{p}^{\mathrm{LLM}}_t = [P(A), P(B), P(C)]$. The EFR vector is:
\[
\boxed{\;\mathbf{p}^{\mathrm{EFR}}_t = \lambda\,\mathbf{p}^{\mathrm{LLM}}_t + (1-\lambda)\,\mathbf{p}^{\mathrm{DCE}}_t\;}
\]
with $\lambda = 0.25$. Because both $\mathbf{p}^{\mathrm{LLM}}_t$ and $\mathbf{p}^{\mathrm{DCE}}_t$ lie on the probability simplex ($\sum_j p_j = 1$, $p_j \geq 0$), the convex combination $\mathbf{p}^{\mathrm{EFR}}_t$ also lies on the simplex and $\sum_j p^{\mathrm{EFR}}_{t,j} = 1$ by construction.
\end{itemize}

\paragraph{Extension from binary formulation.}\label{par:binary-to-multinomial}
The main-text EFR equation $\pi_{\mathrm{EFR}}(y=1\mid\mathbf{x}) = \lambda\,\pi_{\mathrm{LLM}}(y=1\mid\mathbf{x}) + (1-\lambda)\,P_{\mathrm{emp}}(y=1\mid\mathbf{x})$ is a binary policy-versus-opt-out formulation where $P_{\mathrm{emp}}$ is the sigmoid of the utility gap. For the exact-task categorical validation, EFR is extended from this binary formulation to the full three-alternative probability simplex by convexly combining the LLM and training-split DCE probability vectors as defined above. The binary and vector formulations are equivalent under IIA when the choice set contains only one non-opt-out alternative and the opt-out; with two non-opt-out alternatives (A, B) plus opt-out (C), the softmax vector is the natural generalization.
...
\paragraph{Qwen2.5-72B-Instruct local implementation.} This validation uses a local replication with Qwen2.5-72B via Ollama (model: \texttt{qwen2.5:72b}, quantization: Q4\_K\_M, Ollama version 0.3.0). Inference parameters: temperature 0.7, top-p 1.0, max tokens 512, 10 repetitions per task. The inference condition differs from the original Hugging Face endpoint (Table S2) in quantization and sampling implementation; exact numerical correspondence is not guaranteed. Endpoint and model identifiers for DeepSeek V4 Pro and MiroThinker-1.7-mini are reported in Table S2.

\paragraph{Prompt template (exact-task multinomial validation).}
The respondent-facing multinomial prompt for the six canonical DCE tasks is:

\begin{verbatim}
You are evaluating a vaccination decision. The respondent is deciding 
whether to accept the proposed COVID-19 vaccine. Read the policy 
description below and decide.

Alternative A
Waiting time: [VALUE] months
Vaccine effectiveness: [VALUE]%
Risk of side effects: [VALUE]%
Cash incentive: [VALUE] RMB
Vaccine origin: [Domestic/Imported]
...
What is the probability that the respondent chooses each alternative?
Report P(A), P(B), and P(C) as probabilities between 0 and 1 that sum to 1.
Format: P(A)=[value], P(B)=[value], P(C)=[value]
\end{verbatim}

\paragraph{Parser and normalization rules.}
\begin{enumerate}
    \item \textbf{Extraction:} Parse P(A), P(B), P(C) using regex pattern \texttt{P\\((A|B|C)\\)\\s*=\\s*([0-9.]+)}.
    \item \textbf{Validation:} Check that all three probabilities are present and parseable. If any missing, mark as \texttt{PARSE\_FAIL}.
    \item \textbf{Sum check:} If $|P(A)+P(B)+P(C) - 1.0| > 0.01$, apply proportional normalization: $P'(k) = P(k) / \sum_j P(j)$. In the analyzed outputs, no normalization was required (all probabilities summed to 1.0 within floating-point precision).
    \item \textbf{Clipping:} Final probabilities clipped to $[0.001, 0.999]$ to avoid log(0).
    \item \textbf{Position bias check:} Alternatives A/B presentation order matches the original DCE questionnaire. No counterbalancing was performed for the primary validation because the empirical DCE frontier is order-invariant. To assess potential LLM-specific position bias, we conducted a sensitivity experiment swapping A/B labels on the six canonical tasks (60 additional generations). The Raw LLM log loss changed from 1.0200 (original order) to 1.0307 (swapped order), a relative difference of 1.05\%, suggesting minimal position bias for the evaluated models.
\end{enumerate}

\paragraph{Missing output handling.}
If the LLM fails to generate a parseable response (e.g., refusal, malformed output), the response is marked as \texttt{PARSE\_FAIL} and excluded from the mean calculation. In practice, parse failures were rare ($<1\%$ for Qwen2.5-72B).
