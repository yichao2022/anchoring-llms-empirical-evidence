...
\emph{(v) Auditability and explainability.} EFR provides an auditable anchor ($\pi_{\mathrm{EFR}}$ is a deterministic convex combination), but the LLM component remains a black box. Users must trust the LLM-generated probabilities before they enter the EFR blend; the framework does not eliminate LLM opacity, it only bounds its deviation from empirical behavior. This creates a tension: the regularization layer is transparent, but the input layer is not.
