...
EFR trades exact-task predictive performance for narrative-policy evaluation capability (multinomial log loss: Raw LLM $1.1949$; EFR $1.2074$; Pure-DCE $1.2178$; uniform benchmark $1.0986$). \textbf{This negative result is reported transparently:} EFR did not improve exact-task predictive performance relative to Raw LLM, and all three model-based approaches exceeded the uniform-probability benchmark. The NDS pilot ($n=2$) is reported as illustrative mechanism evidence only: isotonic NDS $\rho = -0.943$ vs. EFR $\rho = +0.894$ for DeepSeek V4. \textbf{Figure~\ref{fig:heldout_predictive}} reports the exact-task categorical held-out validation results.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\textwidth]{figures/figure_1_heldout_predictive.png}
\caption{Exact-task categorical held-out validation multinomial log loss (Pure-DCE: $1.2178$; EFR: $1.2074$; Raw LLM: $1.1949$; uniform benchmark: $1.0986$) on six canonical DCE tasks ($N=205$ held-out respondents, 1,230 respondent-task observations). \textbf{This negative result is reported transparently:} all three model-based approaches exceeded the uniform-probability benchmark, and EFR did not improve predictive performance relative to Raw LLM.}
