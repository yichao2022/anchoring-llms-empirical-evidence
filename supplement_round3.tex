...
To assess the representativeness of the 21.9\% matched held-out subset ($N=808$ \emph{alternative-level binary-outcome rows} from $N=205$ held-out respondents (each row is one of three alternatives within one of six tasks; the held-out pool contains $N=3{,}690$ alt-rows; after excluding one anomalous wait=2 alt-row, 3{,}689 rows remain under the intended design, of which 808 are matched and 2{,}876 are unmatched)) used in Table~\ref{tab:S7} of this supplement, we compare its covariate distribution against the unmatched held-out alt-row subset ($N=2,881$). Table~\ref{tab:S15} reports the means and Standardized Mean Differences (SMD) for key DCE attributes and the choice rate.
...
All 1,027 respondents completed the same six choice tasks, each presenting two non-opt-out alternatives (A, B) and an opt-out alternative (C). The six administered tasks comprised 12 unique non-opt-out profiles, covering 12 of 216 candidate profile cells (5.6\%). A post-hoc dominance audit of the six A-versus-B profile pairs confirmed no strict dominance across the five monotonic attributes; two tasks (Tasks 1 and 3) exhibited near-dominance with one alternative winning on four of five attributes, but each retained at least one attribute favoring the other alternative. We hold out 20\% of respondents ($N = 205$) using a seeded random split (seed = 2026; 822 training, 205 test) and use their choices on the six canonical tasks for validation. Exact log loss values are reported in Table~\ref{tab:S16}.
