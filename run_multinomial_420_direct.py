#!/usr/bin/env python3
"""
Execute 420 Qwen calls for 42 DCE tasks (multinomial validation) using mlx_lm directly.
...
MODEL_PATH = "/Volumes/ORICO/mlx-models/Qwen2-72B-4bit"

def load_tasks(path: Path) -> list[dict]:
    with open(path, newline="", encoding="utf-8") as f:
        return list(csv.DictReader(f))

def call_qwen_direct(model, tokenizer, prompt: str, task_num: int, rep: int) -> dict:
    """Call mlx_lm directly."""
    try:
        # Format for chat
        messages = [
        ...
        # Generate (mlx_lm generate doesn't support temperature)
        response = generate(
            model, 
            tokenizer, 
            prompt=text,
...
def main():
    ...
    for task in tasks:
        ...
        for rep in range(1, 11):
            ...
            result = call_qwen_direct(model, tokenizer, prompt, task_num, rep)
